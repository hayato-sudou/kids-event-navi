...
{/* タスクリスト */}
          ...
          >
            {tasks.map((task) => {
              const hasDetail = !!task.description;
              const isExpanded = expandedId === task.id;
              const detailId = `task-detail-${task.id}`;

              return (
                <li key={task.id} className="py-1 group">
                  {/* 1行目: チェックボックス + 概要 + 削除 */}
                  <div className="flex items-center gap-3 py-1">
                    ...
                    {/* 概要: 詳細があれば開閉トグルを兼ねるボタン、なければ従来どおり */}
                    {hasDetail ? (
                      <button
                        onClick={() => toggleDetail(task.id)}
                        aria-expanded={isExpanded}
                        aria-controls={detailId}
                        className={`flex-1 flex items-center gap-1.5 text-left text-sm transition-colors ${
                          task.checked ? 'line-through text-stone-300' : 'text-stone-700'
                        }`}
                      >
                        <span className="flex-1">{task.label}</span>
                        <svg
                          className={`w-3.5 h-3.5 text-stone-300 flex-shrink-0 transition-transform ${
                  ...
                  {/* 2行目: 詳細パネル（開いているときのみ表示） */}
                  {hasDetail && isExpanded && (
                    <p
                      id={detailId}
                      className="ml-8 mr-6 mb-1 px-3 py-2 text-xs leading-relaxed
                                 text-stone-500 bg-stone-50 rounded-lg"
                    >
                      {task.description}
                    </p>
                  )}
